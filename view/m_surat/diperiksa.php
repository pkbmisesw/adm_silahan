...
<?php
$title = "Silahan Kawan | Surat Diperiksa";
                                    ...
                                    <table id="datatable" class="table table-bordered dt-responsive nowrap"
                                        ...
                                        <tbody>
                                            ...
                                            if ($_SESSION['level_id'] == 3) {
                                                $sql = $conn->prepare("SELECT m_surat.*, m_user.nama as nama_user FROM `m_surat` INNER JOIN `m_user` ON m_surat.user_id = m_user.id WHERE status=1 AND petugas_id=:petugas_id ORDER BY id DESC");
                                                $sql->execute([":petugas_id" => $_SESSION['user_id']]);
                                            }

                                            if ($_SESSION['level_id'] == 5) {
                                                $sql = $conn->prepare("SELECT m_surat.*, m_user.nama as nama_user FROM `m_surat` INNER JOIN `m_user` ON m_surat.user_id = m_user.id WHERE status=1 AND user_id=:user_id ORDER BY id DESC");
                                                    ...
                                                    <?php if ($_SESSION['level_id'] == 3) { ?>
                                                        <td>
                                                            <button data-id="<?= $data['id'] ?>"
                                                                data-note="<?= $data['note'] ?>" type="button"
                                                                class="btn btn-light btn_update_operator"
                                                                data-toggle="modal">✎</button>
                                                            <a class="btn btn-success"
                                                                href="../../controller/<?php echo $dba; ?>_controller.php?op=disposisi&id=<?php echo $data['id'] ?>"
                                                                onclick="return confirm('Apakah anda yakin ingin mendisposisikan permohonan ini?');">&#x2713;</a>
                                                            <a class="btn btn-danger"
                                                                href="../../controller/<?php echo $dba; ?>_controller.php?op=deny&id=<?php echo $data['id'] ?>"
                                                                onclick="return confirm('Apakah anda yakin ingin menolak permohonan ini?');">X</a>
                                                        </td>
                                                    <?php } ?>
                                                </tr>
                                                <?php $count++;
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="note_edit" name="note"
                                        placeholder="Silahkan Mengisi Catatan" />
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
