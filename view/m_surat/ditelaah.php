...
<?php
$title = "Silahan Kawan | Surat Ditelaah";include('../head_table.php');
?>
                            ...
                            <div class="card-body">
                                ...
                                <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    ...
                                    if($_SESSION['level_id'] == 3){
                                        $sql = $conn->prepare("SELECT m_surat.*, m_user.nama as nama_user FROM `m_surat` INNER JOIN `m_user` ON m_surat.user_id = m_user.id WHERE status=3 AND petugas_id=:petugas_id ORDER BY id DESC");
                                        $sql->execute([":petugas_id" => $_SESSION['user_id']]);
                                    }

                                    if ($_SESSION['level_id'] == 5) {
                                        $sql = $conn->prepare("SELECT m_surat.*, m_user.nama as nama_user FROM `m_surat` INNER JOIN `m_user` ON m_surat.user_id = m_user.id WHERE status=3 AND user_id=:user_id ORDER BY id DESC");
